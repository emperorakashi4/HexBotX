...
from sql_helper.pmpermit_sql import is_approved, approve, disapprove, get_all_approved
...
BAALAJI_TG_USER_BOT = "`My Master Hasn't Approved You To PM. Don't Spam.`"
...
UNIBORG_USER_BOT_NO_WARN = "`Hi! I Will Answer To Your Message Soon. Please Wait For My Response And Don't Spam My PM.`"
    ...
    if Config.NO_P_M_SPAM:
        if event.is_private:
            if not is_approved(chat.id):
                ...
                await event.edit("`User Approved To PM!`")
                await asyncio.sleep(3)
                await event.delete()
    ...
    if Config.NO_P_M_SPAM:
        if event.is_private:
            if is_approved(chat.id):
                disapprove(chat.id)
                await event.edit("`You've Been Blocked And Reported!`")
                await asyncio.sleep(3)
                await borg(functions.contacts.BlockRequest(chat.id))
