...
USER_BOT_WARN_ZERO = "`You Have Been Blocked And Reported!`"
USER_BOT_NO_WARN = ("`Hello!`"
                    "`You Are In `"
                    f"{DEFAULTUSER}'s` Inbox.`\n\n"
                    "`I Haven't Approved You To PM Yet!`\n\n"
                    "`Please Wait Till I Respond!\nAnd Don't Spam!`")
    ...
    async def block_p_m(event):
        ...
        if event.is_private:
            if pmpermit_sql.is_approved(chat.id):
                ...
                await event.edit("`You Have Been Blocked And Reported` [{}](tg://user?id={})".format(firstname, chat.id))
                await asyncio.sleep(3)
