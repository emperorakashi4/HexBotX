# Copyright (C) 2020 The HexBot Company LLC.
#
# Licensed under the HexBot Public License, Version 1.c (the "License");
...
async def _(event):
    ...
    try:
        ...
        await event.edit(f"User {input_cmd}ned Successfully!")
...
async def gspider(gspdr):
    ...
    if gmute(user.id) is False:
        await gspdr.edit(
            '`Error! User is already Gmuted!`')
    else:
        if reason:
            await gspdr.edit(f"`Gmuted Successfully!`Reason: {reason}")
        else:
            await gspdr.edit("`Gmuted Successfully!`")
...
async def rm_deletedacc(show):
    ...
    if con != "clean":
        await show.edit("`Searching for dead accounts...`")
    ...
    await show.edit("`Deleting dead accounts..`")
    del_u = 0
    del_a = 0
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                await show.edit("`I don't have sufficient rights!`")
                return
